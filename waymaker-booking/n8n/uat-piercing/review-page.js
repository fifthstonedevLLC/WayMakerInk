...
/* Session lengths, priced at the artist's hourly rate. Both halves of this
   must stay in step with nodes/intake.js — that node priced the buttons in the
   email, this one re-prices the same tier from the sheet row's artistKey, and
   the artist would notice immediately if tapping "2 hr · $200" opened a page
   offering $250.

   The tier link carries no price, and it deliberately doesn't: `sig` covers
   rid.action.exp only, so a price in the URL would be the one field in it a
   client could edit. The rate table is the authority at both ends. */
const RATES = {
  nic: cfg('WM_ARTIST_NIC_RATE'),
  laynie: cfg('WM_ARTIST_LAYNIE_RATE')
};

const TIERS = {
  tier1: { label: '1 hr',           hours: 1 },
  tier2: { label: '2 hr',           hours: 2 },
  tier3: { label: '3 hr',           hours: 3 },
  tier4: { label: '4 hr',           hours: 4 },
  tier5: { label: 'All Day · 7 hr', hours: 7 }
};
...
/* Priced only once past the gate above: an already-decided request should show
   the "already handled" page even if its artistKey is somehow unreadable,
   rather than throwing a raw n8n error at the artist. */
let quote = '';

if (!isDecline) {
  const artistKey = String(row.artistKey || '').trim().toLowerCase();
  const rate = Number(RATES[artistKey]);
  if (!Number.isFinite(rate) || rate <= 0) {
    throw new Error(
      `No hourly rate for artist "${artistKey}" (rid ${link.rid}). ` +
      `Add them to RATES here and to ARTISTS in nodes/intake.js.`
    );
  }
  quote = `${tier.label} · ${money(rate * tier.hours)}`;
}

const heading = isDecline
  ? 'Decline this request?'
  : `Send ${esc(quote)} to ${esc(row.firstName)}?`;
...
const decisionFields = isDecline
  ...
  : `<div class="field">
       ...
       <input id="estimate" name="estimate" value="${esc(quote)}">
     </div>
     <div class="field">
...
const html = shell(`
  ...
  <!-- ⚠ UAT COPY — \`-uat\` suffix. Production is \`/webhook/decide-commit\`.
       Revert on promotion; see the note in intake.js decisionUrl(). -->
  <form method="POST" action="${BASE}/webhook/decide-commit-uat">
