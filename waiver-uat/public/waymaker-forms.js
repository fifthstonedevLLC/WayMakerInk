document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Submit to n8n --------------------------------------------------------
  // Same-origin. nginx proxies this to the n8n webhook and injects the shared
  // secret server-side (see nginx.conf.template), so the browser never holds a
  // credential and there is no CORS preflight. Do not put the n8n URL here —
  // it belongs in WM_WAIVER_WEBHOOK_URL in the Dokploy environment.
  const WEBHOOK_URL = '/api/waiver';
  ...
  // ---- Turnstile ------------------------------------------------------------
  // The site key is public by design, but it is domain-bound — UAT and
  // production need different keys — so it comes from config.js (written at
  // container start from WM_TURNSTILE_SITE_KEY) rather than being baked into
  // the HTML. That keeps promotion a config change, not a code diff.
  //
  // We inject Cloudflare's script ourselves and render explicitly in its onload
  // callback. Auto-render (class="cf-turnstile" + data-sitekey in the markup)
  // would race: the key only exists once our own deferred script has run.
  const TURNSTILE_SITE_KEY =
    (window.WM_CONFIG && window.WM_CONFIG.turnstileSiteKey) || '';
  const turnstileMount = document.querySelector('[data-turnstile]');
  let turnstileWidgetId = null;

  if (form && turnstileMount && TURNSTILE_SITE_KEY) {
    window.wmTurnstileReady = () => {
    ...
    s.src = 'https://challenges.cloudflare.com/turnstile/v0/api.js?onload=wmTurnstileReady';
    s.async = true;
    s.defer = true;
    document.head.appendChild(s);
  }

  // Turnstile writes its token into an input named cf-turnstile-response, which
  // FormData sweeps up with everything else. Every access below is guarded: if
  // Cloudflare is unreachable window.turnstile is never defined, and an
  // unguarded call would throw inside the submit handler and take the form down.
  const turnstileField = () => (form && form.elements ? form.elements['cf-turnstile-response'] : null);
  const turnstileToken = () => {
    const el = turnstileField();
    return el && typeof el.value === 'string' ? el.value.trim() : '';
  };
  const resetTurnstile = () => {
    try {
      if (window.turnstile && typeof window.turnstile.reset === 'function') {
        // Reset the widget we rendered by id — a bare reset() targets the first
        // widget on the page, which is not necessarily ours.
        window.turnstile.reset(turnstileWidgetId || undefined);
      }
    } catch (_) { /* widget not ready; the next submit re-checks the token */ }
  };

  // Client-facing copy for a failed submit. Deliberately says nothing about the
  // backend — the endpoint, the proxy and the status text all stay server-side.
  const errorMessageFor = (status) => {
    if (status === 429) {
      return 'Too many submissions from this connection. Please wait a minute and try again.';
    }
    if (status === 403) {
      return 'We couldn\'t verify this submission. Please complete the "I\'m human" check again, then resubmit.';
    }
    if (status === 413) {
      return 'The photos attached are too large to submit. Please retake the ID photo and try again.';
    }
    return 'Sorry, something went wrong submitting your waiver. Please try again or ask your artist for help.';
  };
  ...
  if (form) {
    form.addEventListener('submit', async (event) => {
      ...
      //
      // This sweeps up the `company` honeypot too, and that is deliberate — do
      // NOT add a client-side check for it here. The booking form checks it in
      // the browser and deletes it from the payload, so its server-side check
      // sees nothing and a bot POSTing past the page is unfiltered. The server
      // decides; see n8n/nodes/validate-waiver.js.
      ...
      // Turnstile: no token means the widget hasn't been solved (or hasn't
      // loaded). Route it through flagMissing so it gets the same banner and
      // scroll-to-field treatment as every other required field. The token
      // itself rides along in `data` and is verified server-side — this check
      // is a courtesy to the client, not the control.
      if (!turnstileToken()) {
        flagMissing('The "I\'m human" verification', document.querySelector('.cf-turnstile'));
      }
      ...
      try {
        ...
        if (!res.ok) {
          // Carry the status so the catch below can explain what happened.
          // The response body is deliberately not surfaced to the client.
        ...
        form.innerHTML =
          ...
          '<p><a href="index.html">Return to the forms home page now</a></p></section>';

        window.setTimeout(() => {
          window.location.assign('index.html');
        }, 3000);
      } catch (err) {
        // A spent Turnstile token can never succeed on retry — tokens are
        // single-use and expire in ~5 minutes. Mint a fresh one before the
        // client tries again, or every retry in this page load fails.
        resetTurnstile();

        showFormError(errorMessageFor(err.status));
        if (submitBtn) {
